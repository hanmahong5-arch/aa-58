 ...
 */
public class PostgreSQLPlayerEventsWindowDAO extends PlayerEventsWindowDAO {

    private static final Logger log = LoggerFactory.getLogger(PostgreSQLPlayerEventsWindowDAO.class);
    
    private static final String SELECT_QUERY = "SELECT * FROM \"player_events_window\" WHERE \"account_id\"=?";
    private static final String INSERT_QUERY = "INSERT INTO \"player_events_window\" (\"account_id\", \"event_id\", \"last_stamp\", \"elapsed\") VALUES(?,?,?,?) ON CONFLICT (account_id, event_id) DO UPDATE SET last_stamp = EXCLUDED.last_stamp";
