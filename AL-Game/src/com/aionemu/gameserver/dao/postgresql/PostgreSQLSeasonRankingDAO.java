 ...
 */
public class PostgreSQLSeasonRankingDAO extends SeasonRankingDAO {

    private static final Logger log = LoggerFactory.getLogger(PostgreSQLSeasonRankingDAO.class);

    public static final String SELECT_PLAYERS_RANKING = "SELECT competition_ranking.rank AS cr_rank, "
        + "competition_ranking.last_rank AS cr_last_rank, competition_ranking.points AS cr_points, "
        + "competition_ranking.player_id, players.name AS player_name, "
        + "players.id AS player_id2, players.player_class, players.race "
        ...
        try (Connection con = DatabaseFactory.getConnection();
            ...
            try (ResultSet resultSet = stmt.executeQuery()) {
                while (resultSet.next()) {
                    ...
                    int rank = resultSet.getInt("cr_rank");
                    int last_rank = resultSet.getInt("cr_last_rank");
                    int points = resultSet.getInt("cr_points");
                    ...
                    String raceStr = resultSet.getString("race");
