 ...
 */
public class PostgreSQLLegionMemberDAO extends LegionMemberDAO {

    private static final Logger log = LoggerFactory.getLogger(PostgreSQLLegionMemberDAO.class);
    ...
    private static final String SELECT_LEGIONMEMBEREX_QUERY = "SELECT players.name AS player_name, players.exp AS player_exp, "
        + "players.player_class, players.last_online, players.world_id, "
        + "legion_members.legion_id AS lm_legion_id, legion_members.player_id, "
        + "legion_members.\"rank\" AS lm_rank, legion_members.nickname, "
        + "legion_members.selfintro, legion_members.challenge_score "
        + "FROM players INNER JOIN legion_members ON players.id = legion_members.player_id "
        + "WHERE players.id = ?";
    private static final String SELECT_LEGIONMEMBEREX2_QUERY = "SELECT players.id AS player_id, players.exp AS player_exp, "
        + "players.player_class, players.last_online, players.world_id, "
        + "legion_members.legion_id AS lm_legion_id, legion_members.\"rank\" AS lm_rank, "
        + "legion_members.nickname, legion_members.selfintro, legion_members.challenge_score "
        + "FROM players INNER JOIN legion_members ON players.id = legion_members.player_id "
        + "WHERE players.name = ?";
        ...
        try (Connection con = DatabaseFactory.getConnection();
            ...
            try (ResultSet resultSet = stmt.executeQuery()) {
                if (resultSet.next()) {
                    ...
                    legionMemberEx.setName(resultSet.getString("player_name"));
                    legionMemberEx.setExp(resultSet.getLong("player_exp"));

                    try {
                        legionMemberEx.setPlayerClass(PlayerClass.valueOf(resultSet.getString("player_class")));
                    } catch (IllegalArgumentException e) {
                        log.warn("Invalid player class for player {}: {}", playerObjId, resultSet.getString("player_class"));
                    }

                    legionMemberEx.setLastOnline(resultSet.getTimestamp("last_online"));
                    legionMemberEx.setWorldId(resultSet.getInt("world_id"));

                    int legionId = resultSet.getInt("lm_legion_id");

                    try {
                        legionMemberEx.setRank(LegionRank.valueOf(resultSet.getString("lm_rank")));
                    } catch (IllegalArgumentException e) {
                        log.warn("Invalid legion rank for player {}: {}", playerObjId, resultSet.getString("lm_rank"));
                        legionMemberEx.setRank(LegionRank.VOLUNTEER);
                    }

                    legionMemberEx.setNickname(resultSet.getString("nickname"));
                    legionMemberEx.setSelfIntro(resultSet.getString("selfintro"));
        ...
        try (Connection con = DatabaseFactory.getConnection();
            ...
            try (ResultSet resultSet = stmt.executeQuery()) {
                if (resultSet.next()) {
                    ...
                    legionMember.setObjectId(resultSet.getInt("player_id"));
                    legionMember.setExp(resultSet.getLong("player_exp"));
                    ...
                    try {
                        legionMember.setRank(LegionRank.valueOf(resultSet.getString("lm_rank")));
                    } catch (IllegalArgumentException e) {
                        log.warn("Invalid legion rank for player {}: {}", playerName, resultSet.getString("lm_rank"));
                        legionMember.setRank(LegionRank.VOLUNTEER);
                    }
