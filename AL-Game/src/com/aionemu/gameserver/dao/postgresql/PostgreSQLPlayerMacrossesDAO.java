 ...
 */
public class PostgreSQLPlayerMacrossesDAO extends PlayerMacrossesDAO {

	private static Logger log = LoggerFactory.getLogger(PostgreSQLPlayerMacrossesDAO.class);
	...
	public void addMacro(final int playerId, final int macroPosition, final String macro) {
		try (Connection con = DatabaseFactory.getConnection();
			...
			log.debug("[DAO: PostgreSQLPlayerMacrossesDAO] storing macro " + playerId + " " + macroPosition);
	...
	public void updateMacro(final int playerId, final int macroPosition, final String macro) {
		try (Connection con = DatabaseFactory.getConnection();
			...
			log.debug("[DAO: PostgreSQLPlayerMacrossesDAO] updating macro " + playerId + " " + macroPosition);
	...
	public void deleteMacro(final int playerId, final int macroPosition) {
		try (Connection con = DatabaseFactory.getConnection();
			...
			log.debug("[DAO: PostgreSQLPlayerMacrossesDAO] removing macro " + playerId + " " + macroPosition);
			stmt.setInt(1, playerId);
			stmt.setInt(2, macroPosition);
			stmt.executeUpdate();
		} catch (SQLException e) {
	...
	public MacroList restoreMacrosses(final int playerId) {
		...
		try (Connection con = DatabaseFactory.getConnection();
			...
			stmt.setInt(1, playerId);
			log.debug("[DAO: PostgreSQLPlayerMacrossesDAO] loading macroses for playerId: " + playerId);
