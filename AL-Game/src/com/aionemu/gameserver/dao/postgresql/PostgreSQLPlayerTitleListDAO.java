...
/**
 * PostgreSQL implementation of PlayerTitleListDAO
 * @author xavier
 */
public class PostgreSQLPlayerTitleListDAO extends PlayerTitleListDAO {

    private static final Logger log = LoggerFactory.getLogger(PostgreSQLPlayerTitleListDAO.class);

    private static final String LOAD_QUERY = "SELECT \"title_id\", \"remaining\" FROM \"player_titles\" WHERE \"player_id\" = ? ORDER BY \"title_id\"";
    private static final String INSERT_QUERY = "INSERT INTO \"player_titles\" (\"player_id\", \"title_id\", \"remaining\") VALUES (?, ?, ?) " + "ON CONFLICT (player_id, title_id) DO UPDATE SET remaining = EXCLUDED.remaining";
