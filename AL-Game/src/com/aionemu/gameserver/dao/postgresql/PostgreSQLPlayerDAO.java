...
/**
 * PostgreSQL implementation of PlayerDAO
 * Fixed connection leaks - removed all DB.insertUpdate() and DB.prepareStatement()
 */
public class PostgreSQLPlayerDAO extends PlayerDAO {
    
    private static final Logger log = LoggerFactory.getLogger(PostgreSQLPlayerDAO.class);
    ...
    private static final String SELECT_PLAYER_NAMES_BASE = "SELECT id, \"name\" FROM players WHERE id IN (";
    ...
    private static final String SELECT_PLAYERS_TO_DELETE_QUERY = "SELECT id FROM players WHERE last_online < CURRENT_DATE - CAST(? || ' days' AS INTERVAL)";
    ...
    public Map<Integer, String> getPlayerNames(Collection<Integer> playerObjectIds) {
        ...
        // Build parameterized IN clause: "SELECT ... WHERE id IN (?, ?, ?)"
        StringBuilder sb = new StringBuilder(SELECT_PLAYER_NAMES_BASE);
        for (int i = 0; i < playerObjectIds.size(); i++) {
            if (i > 0) {
                sb.append(", ");
            }
            sb.append('?');
        ...
        try (Connection con = DatabaseFactory.getConnection();
            ...
            int idx = 1;
            for (Integer id : playerObjectIds) {
                s.setInt(idx++, id);
            }
    ...
    public List<Integer> getPlayersToDelete(final int daysOfInactivity, int limitation) {
        boolean hasLimit = limitation > 0;
        String sql = hasLimit
                ? SELECT_PLAYERS_TO_DELETE_QUERY + " LIMIT ?"
                : SELECT_PLAYERS_TO_DELETE_QUERY;
