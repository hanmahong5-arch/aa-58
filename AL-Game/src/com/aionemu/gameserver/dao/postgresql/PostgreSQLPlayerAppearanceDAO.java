...
public class PostgreSQLPlayerAppearanceDAO extends PlayerAppearanceDAO {

    private static final Logger log = LoggerFactory.getLogger(PostgreSQLPlayerAppearanceDAO.class);
    
    private static final String SELECT_QUERY = "SELECT * FROM player_appearance WHERE player_id = ?";
    private static final String REPLACE_QUERY = "INSERT INTO player_appearance ("
        + "player_id, voice, skin_rgb, hair_rgb, eye_rgb, lip_rgb, face, hair, deco, tattoo, "
        + "face_contour, expression, pupil_shape, remove_mane, right_eye_rgb, eye_lash_shape, "
        ...
        + "ear_shape, head_size, neck, neck_length, shoulder_size, torso, chest, waist, hips, "
        + "arm_thickness, hand_size, leg_thickness, facial_rate, foot_size, arm_length, leg_length, "
        + "shoulders, face_shape, pupil_size, upper_torso, fore_arm_thickness, hand_span, "
        + "calf_thickness, height) VALUES ("
        + "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
        + "?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
        + "?, ?) ON CONFLICT (player_id) DO UPDATE SET "
        + "voice=EXCLUDED.voice, skin_rgb=EXCLUDED.skin_rgb, hair_rgb=EXCLUDED.hair_rgb, "
        + "eye_rgb=EXCLUDED.eye_rgb, lip_rgb=EXCLUDED.lip_rgb, face=EXCLUDED.face, hair=EXCLUDED.hair, "
        + "deco=EXCLUDED.deco, tattoo=EXCLUDED.tattoo, face_contour=EXCLUDED.face_contour, "
        + "expression=EXCLUDED.expression, pupil_shape=EXCLUDED.pupil_shape, remove_mane=EXCLUDED.remove_mane, "
        + "right_eye_rgb=EXCLUDED.right_eye_rgb, eye_lash_shape=EXCLUDED.eye_lash_shape, "
        + "jaw_line=EXCLUDED.jaw_line, forehead=EXCLUDED.forehead, eye_height=EXCLUDED.eye_height, "
        ...
        + "shoulder_size=EXCLUDED.shoulder_size, torso=EXCLUDED.torso, chest=EXCLUDED.chest, "
        + "waist=EXCLUDED.waist, hips=EXCLUDED.hips, arm_thickness=EXCLUDED.arm_thickness, "
        + "hand_size=EXCLUDED.hand_size, leg_thickness=EXCLUDED.leg_thickness, "
        + "facial_rate=EXCLUDED.facial_rate, foot_size=EXCLUDED.foot_size, "
        + "arm_length=EXCLUDED.arm_length, leg_length=EXCLUDED.leg_length, "
        + "shoulders=EXCLUDED.shoulders, face_shape=EXCLUDED.face_shape, "
        + "pupil_size=EXCLUDED.pupil_size, upper_torso=EXCLUDED.upper_torso, "
        + "fore_arm_thickness=EXCLUDED.fore_arm_thickness, hand_span=EXCLUDED.hand_span, "
        + "calf_thickness=EXCLUDED.calf_thickness, height=EXCLUDED.height";
    ...
    public boolean store(final int id, final PlayerAppearance pa) {
        try (Connection con = DatabaseFactory.getConnection();
            ...
            log.debug("[DAO: PostgreSQLPlayerAppearanceDAO] storing appearance {}", id);
