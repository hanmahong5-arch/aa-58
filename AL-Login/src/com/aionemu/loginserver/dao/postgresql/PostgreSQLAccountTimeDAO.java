...
/**
 * PostgreSQL AccountTime DAO implementation
 * 
 * @author
 */
public class PostgreSQLAccountTimeDAO extends AccountTimeDAO {

    private static final Logger log = LoggerFactory.getLogger(PostgreSQLAccountTimeDAO.class);

    @Override
    public boolean updateAccountTime(final int accountId, final AccountTime accountTime) {
        String query = "INSERT INTO account_time (account_id, last_active, expiration_time, "
            + "session_duration, accumulated_online, accumulated_rest, penalty_end) "
            + "VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT (account_id) DO UPDATE SET "
            + "last_active=EXCLUDED.last_active, expiration_time=EXCLUDED.expiration_time, "
